...
export default async function LeasesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
    ...
    const gedComplete = (lease: typeof leases[0]) => {
        ...
        if (!types.includes('BAIL') || !types.includes('EDL')) return false;
        const gt = (lease as any).guarantorType as string | null;
        if (gt === 'PRIVATE') return types.includes('CAUTIONNEMENT');
        if (gt === 'VISALE') return !!((lease as any).guarantorVisaleOk);
        return true;
    };
    ...
    const actionButtons = (lease: typeof leases[0]) => {
        ...
        <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center', justifyContent: 'flex-end' }}>
            <Link
                ...
                style={{ fontSize: '0.8rem', padding: '0.3rem 0.6rem', ...(complete ? { background: 'rgba(34,197,94,0.15)', color: '#22c55e', borderColor: 'rgba(34,197,94,0.3)' } : { background: 'rgba(239,68,68,0.1)', color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }) }}
                title={complete ? 'GED complète (bail + EDL)' : 'GED incomplète — bail et/ou état des lieux manquant'}
            >
                📎{complete ? ' ✓' : ' !'}
            </Link>
                                                ...
                                                <td>
                                                    {lease.depositAmount ? (
                                                        <DepositStatusButton
                                                            ...
                                                            depositPaidAmount={lease.depositPaidAmount} depositReturnedAmount={lease.depositReturnedAmount} depositReturnNote={lease.depositReturnNote}
                                                        />
                                                    ) : '—'}
                                                </td>
                            ...
                            <div className={styles.grid}>
                                {pastLeases.map((lease) => (
                                    <ClickableCard key={lease.id} href={`/leases/${lease.id}`} className={styles.historyCard}>
                                        ...
                                        <div className={styles.cardFooter} style={{ gap: '0.5rem' }}>
                                            <Link href={`/leases/${lease.id}`} className="std-add-button" style={{ fontSize: '0.85rem', padding: '0.4rem 0.75rem', opacity: 0.7, ...(gedComplete(lease) ? { background: 'rgba(34,197,94,0.15)', color: '#22c55e', borderColor: 'rgba(34,197,94,0.3)' } : { background: 'rgba(239,68,68,0.1)', color: '#ef4444', borderColor: 'rgba(239,68,68,0.3)' }) }} title={gedComplete(lease) ? 'GED complète (bail + EDL)' : 'GED incomplète — bail et/ou état des lieux manquant'}>
                                                📎{gedComplete(lease) ? ' ✓' : ' !'}
                                            </Link>
