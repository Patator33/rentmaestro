...
type DocTypeValue = 'BAIL' | 'EDL' | 'AUTRE';
...
export default function LeaseDocumentUpload({ leaseId, initialDocuments }: Props) {
    ...
    const getDocTypeLabel = (dt: string) => DOC_TYPES.find(d => d.value === dt)?.label ?? '📄 Autre';
                        ...
                        <li key={doc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 1rem', background: 'var(--surface)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', minWidth: 0 }}>
                                <span style={{
                                    ...
                                    borderRadius: '4px',
                                    whiteSpace: 'nowrap',
                                    background: doc.docType === 'BAIL' ? 'rgba(43,140,238,0.12)' : doc.docType === 'EDL' ? 'rgba(34,197,94,0.12)' : 'rgba(100,116,139,0.12)',
                                    color: doc.docType === 'BAIL' ? 'var(--primary-color)' : doc.docType === 'EDL' ? '#22c55e' : 'var(--text-muted)',
                                }}>
